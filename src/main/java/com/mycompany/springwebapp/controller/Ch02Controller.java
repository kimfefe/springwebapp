...
/*
	...
	// @PutMapping("/method")
	/*
	 * @RequestMapping(value = "/method", method = RequestMethod.PUT) public void
	 * method3(@RequestBody String json, HttpServletResponse response) throws
	 * Exception { JSONObject jsonObject = new JSONObject(json); String bkind =
	 * jsonObject.getString("bkind"); int bno = jsonObject.getInt("bno");
	 * log.info("bkind: " + bkind); log.info("bno: " + bno);
	 * 
	 * JSONObject root = new JSONObject(); root.put("result", "success"); String
	 * responseJson = root.toString(); //{"result":"success"}
	 * 
	 * response.setContentType("application/json; charset=UTF-8"); PrintWriter pw =
	 * response.getWriter(); pw.print(responseJson); pw.flush(); pw.close(); }
	 */
	...
	@ResponseBody // 이 메소드가 리턴하는 리턴값을 응답 본문에 바로 싣기
	public String ajax3() {
		JSONObject root = new JSONObject();
	...
	public void fileDownload(HttpServletRequest request, HttpServletResponse response) throws Exception { // file을 다운로드 해야하니까 리턴타입으로 void를 지정
	...
	@Auth(Role.ADMIN) // value는 기본 속성 값이라 생략 가능, 로그인한 사용자가 이권한이 있는지 조사하고 권한이 있다면 이 controller의 메소드를
						// 실행한다.
