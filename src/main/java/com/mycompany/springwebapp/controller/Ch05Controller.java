...
public class Ch05Controller {
	...
	public String createCookie(HttpServletResponse response) {
		...
		cookie.setDomain("localhost"); //쿠키 재전송할 서버를 지정
		cookie.setPath("/");		//쿠키를 재전송할 경로를 지정
		cookie.setMaxAge(30*60);	//쿠키의 저장 기간(단위: 초, 파일에 저장), 지정안할 경우: 브라우저 메모리 저장
		cookie.setHttpOnly(true);	//true: 서버만 이용, false: JavaScript에서 접근 허용
		cookie.setSecure(false); 	//false: http, https 모두사용, true: https만 사용
